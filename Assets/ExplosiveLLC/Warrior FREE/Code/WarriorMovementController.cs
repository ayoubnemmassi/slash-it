...
namespace WarriorAnimsFREE
{
    public class WarriorMovementController : SuperStateMachine
    {
        ...
        private void Jump_EnterState()
        {
            ...
            // currentVelocity = (warriorController.superCharacterController.up + transform.forward) * CalculateJumpSpeed(jumpHeight, gravity);
            Vector3 dir = warriorController.inputJumpDirection;
            if (dir == Vector3.zero)
            {
                dir = transform.forward;
            }
            currentVelocity = (warriorController.superCharacterController.up + dir) * CalculateJumpSpeed(jumpHeight, gravity);
